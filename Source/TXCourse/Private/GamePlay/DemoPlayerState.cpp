...
void ADemoPlayerState::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ADemoPlayerState,Rank);
	DOREPLIFETIME(ADemoPlayerState,EndInfo);
}
...
void ADemoPlayerState::OnRep_EndInfo()
{
	UE_LOG(LogTemp,Warning,TEXT("EndCHeck"));
	if(EndInfo.bIsValid)
	{
		UE_LOG(LogTemp,Warning,TEXT("Broad"));
